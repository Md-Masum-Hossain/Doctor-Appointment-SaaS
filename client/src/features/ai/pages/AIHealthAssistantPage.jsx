...
export function AIHealthAssistantPage() {
  ...
  const latestMedicalInsights = latestAIMessage?.medicalInsights || null
  const latestSpecialization = latestMedicalInsights?.recommendedSpecialization || null
  ...
  const latestTips = latestMedicalInsights?.tips || []
  const latestPossibleCauses = latestMedicalInsights?.possibleCauses || []
  const showMedicalSummary = Boolean(latestMedicalInsights)
            ...
            >
              ...
              {showMedicalSummary && latestAIMessage ? (
                ...
                <div className="mt-4 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-600">
                  Conversation mode is active. Ask naturally, and I’ll stay conversational until you switch to a health-related question.
                </div>
              )}
            </motion.section>
